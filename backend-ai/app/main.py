...
async def startup_checks():
    ...
    Do not make network/LLM requests during application startup.
    NVIDIA NIM diagnostics should be triggered explicitly through
    /health/llm instead of blocking FastAPI readiness.
    """
    import logging
    ...
    log.info("============================================================")
    log.info("Research Synthesis Assistant startup")
    log.info(f"LLM provider: {settings.llm_provider}")
    log.info(f"Primary model: {settings.llm_model}")
    log.info(f"Analysis model: {settings.llm_analysis_model}")
    log.info(f"Synthesis model: {settings.llm_synthesis_model}")
    log.info("LLM network diagnostics skipped during startup.")
    log.info("============================================================")
...
async def health_llm():
    """Diagnostic endpoint for LLM status without exposing API keys."""
    provider = settings.llm_provider.lower()
    return {
        "provider": provider,
        "primary_model": settings.llm_model,
        "analysis_model": settings.llm_analysis_model or settings.llm_model,
        "synthesis_model": settings.llm_synthesis_model or settings.llm_model,
        "request_status": "available" if settings.nvidia_nim_api_key else "missing_api_key",
        "status_code": 200,
    }
