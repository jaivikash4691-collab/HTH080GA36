...
class LLMConfig:
    ...
    max_tokens: int = field(default_factory=lambda: _environment_int("LLM_MAX_TOKENS", 4096))
    temperature: float = field(
        default_factory=lambda: _environment_float("LLM_TEMPERATURE", 0.1)
    )
    timeout: int = field(default_factory=lambda: _environment_int("LLM_TIMEOUT", 120))
    max_retries: int = field(
        default_factory=lambda: _environment_int("LLM_MAX_RETRIES", 3)
    )
...
class LLMService:
    ...
    async def complete(
        ...
        """Return one completion, retrying rate limits, 503s, and timeouts."""
        ...
        for attempt in range(1, total_attempts + 1):
            try:
                ...
                logger.warning(
                    "OpenRouter request timed out on attempt %s/%s.", attempt, total_attempts
                )
                if attempt == total_attempts:
                    raise LLMServiceError(
                        f"OpenRouter request timed out after {attempt} attempt(s).",
                        attempts=attempt,
                        max_retries=self.config.max_retries,
                        retryable=True,
                    ) from exc
                ...
                logger.exception("OpenRouter HTTP client error on attempt %s/%s.", attempt, total_attempts)
                raise LLMServiceError(
                    f"OpenRouter HTTP client error: {exc}",
                    attempts=attempt,
                    max_retries=self.config.max_retries,
                ) from exc

        raise AssertionError("Completion retry loop ended unexpectedly")
