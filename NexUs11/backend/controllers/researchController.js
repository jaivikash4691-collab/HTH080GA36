...
export const researchController = {
  ...
  async discover(req, res, next) {
    try {
      ...
      let results = [];
      try {
        const { crossrefService } = await import('../services/crossrefService.js');
        results = await crossrefService.searchCrossref(query);
      } catch (upstreamErr) {
        console.error(`[Research Discovery] Crossref API Request Failed:`, upstreamErr.message);
