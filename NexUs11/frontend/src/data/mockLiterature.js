...
export const MOCK_EVIDENCE_CHUNKS = {};
...
export function generateDynamicSynthesis(papers = []) {
  ...
  const findings = papers.slice(0, 4).map((p, idx) => ({
    ...
    statement: `Evidence from ${p.code || `P${idx + 1}`} establishes baseline validation using ${p.method || 'proposed methodology'}.`,
  ...
  const radarItems = [
    {
      ...
      auditTrail: {
        ...
        counterEvidence: 'No contrary evidence detected in uploaded papers',
  ...
  const lineage = [
    ...
    },
    {
      ...
      detail: `NEXUS strategy engine synthesized multi-paper evidence into a structured experimental direction.`,
    },
  ];
  ...
  const brain = {
    ...
    coreTakeaway: `Evidence synthesized across: ${papers.map((p) => p.title).slice(0, 3).join('; ')}.`,
