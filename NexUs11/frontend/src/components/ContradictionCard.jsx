...
export const ContradictionCard = ({ contradiction }) => {
  const { openEvidence, openPaperProfile, papers } = useResearch();
  ...
  return (
    <div className="glass-card overflow-hidden border-t-4 border-t-[#E11D48] p-6 space-y-6">
      ...
      <div className="pt-2 border-t border-[#E2E8F0] flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          type="button"
          onClick={() => openEvidence(contradiction.evidenceIds?.[0] || 'P1-p7')}
          className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold rounded-lg bg-[#E11D48] text-white hover:bg-[#E11D48]/90 transition-all cursor-pointer shadow-xs"
        >
