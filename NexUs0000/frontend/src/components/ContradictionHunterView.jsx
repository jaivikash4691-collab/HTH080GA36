...
export const ContradictionHunterView = () => {
  const { contradictionHunterItems, papers } = useResearch();

  const items = contradictionHunterItems || [];

  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-[#E2E8F0] dark:border-[#334155] pb-4">
        <div>
          <span className="text-xs font-bold uppercase tracking-wider text-[#E11D48] bg-rose-50 dark:bg-rose-950/40 px-2.5 py-0.5 rounded-full">
            Discrepancy Engine • Cross-Study Divergence
          </span>
          <h2 className="text-2xl font-black text-[#1E1B4B] dark:text-white tracking-tight mt-1">
      ...
      {items.length === 0 ? (
        <div className="glass-card p-12 text-center space-y-3">
          ...
          <p className="text-xs text-[#64748B] dark:text-[#94A3B8] max-w-sm mx-auto">
            {papers.length < 2
              ? 'Upload 2 or more research papers to compare empirical claims and detect empirical discrepancies.'
