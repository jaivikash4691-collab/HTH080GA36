...
export const GapsPage = ({ onNavigate }) => {
  const { gapRadarItems, openValidateGap, papers } = useResearch();
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black uppercase tracking-wider text-[#D97706] bg-amber-50 dark:bg-amber-950/40 px-2.5 py-0.5 rounded">
                    RESEARCH GAP #{idx + 1} • {item.category}
                  </span>
                </div>

                <div className="space-y-1">
                  <h3 className="text-base font-black text-[#1E1B4B] dark:text-white">
                    {item.title}
                  </h3>
                ...
                {/* Supporting Papers */}
                {paperCodes.length > 0 && (
                  <div className="p-3 rounded-xl bg-[#FBF9F5] dark:bg-[#0F172A] border border-[#E2E8F0] dark:border-[#334155] space-y-1.5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {paperCodes.map((pid, pidx) => (
                        <CitationPill key={pidx} id={pid} label={pid} />
                      ))}
                    </div>
                  </div>
                )}
                ...
                {/* Action Buttons */}
                <div className="pt-2 border-t border-[#E2E8F0] dark:border-[#334155] flex flex-wrap items-center justify-between gap-2">
                  <button
                    type="button"
