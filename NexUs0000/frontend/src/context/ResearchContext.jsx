...
export const ResearchProvider = ({ children }) => {
  ...
  const pipelineSteps = [
    ...
    { id: 2, title: 'EXTRACTING KEY FINDINGS', description: 'Auditing empirical claims and computational methods' },
  ...
  const addPaper = async (newPaper) => {
    ...
    const paperWithMeta = {
      ...
      mainResult: newPaper.mainResult || 'Extracted and verified key findings from uploaded document.',
      limitation: newPaper.limitation || 'Sample scope and domain boundary constraints.',
      researchProblem: newPaper.researchProblem || newPaper.problem || null,
      potentialResearchDirection: newPaper.potentialResearchDirection || newPaper.researchDirection || null,
      citationsCount: 0,
    };
  ...
  // Evidence UI handlers (stubs for backward compatibility)
  const openEvidence = () => {};
  const closeEvidence = () => {};
    ...
    setTimeout(() => {
      ...
      if (papers.length === 0) {
        ...
        details = papers.slice(0, 3).map((p, idx) =>
          `${p.code || `P${idx + 1}`} ("${p.title}"): ${p.mainResult || 'Empirical findings extracted from uploaded document.'}`
        );
      }
